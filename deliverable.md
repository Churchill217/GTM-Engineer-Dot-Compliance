...
- **Dedup key:** normalized email if `@` present (person); else a per-registration **surrogate** (corporate-domain anchor + a stable row hash) for contact-less rows. A bare domain is **never the key on its own** — two unrelated registrants can share it (the two `vitalant.org` rows), and an upsert on the shared key would destructively merge them. The corporate domain is kept separately as `account_domain` (a non-destructive account hint), never a merge key. Every row gets a stable key → idempotent re-ingest.
...
- **Object type and lead-presence always agree, by construction.** An Account Task is the *contact-less* destination only — it never carries a person record; a named row always writes a Lead/update. That contract is enforced by an in-code invariant (an assert), not just convention, so no branch can ship a contradictory "Account Task with a contact attached" payload.
...
|---|---|---|---|
| **Salesforce daily API** | **100,000 base + 1,000 per user-license** (Enterprise) / **+5,000** (Unlimited), max 1,000,000 / 24h. *Bulk API and the 25-concurrent-long-running limit are separate buckets.* | ~2 calls per red alert (1 suppression read + 1 score write); score writes ride the 2–3×/day **batch**, not per-signal | **A fraction of a percent.** Even the 100k floor ÷ 2 ≈ 50,000 alerts/day of room; we fire tens. |
...
| **Lusha** (fallback) | email 1 credit, **phone 5 (API) + 1/request even on a miss**; 1,500–50,000/day by plan; **API access only on the custom Scale tier** | the EU/mobile phone residue | Fine on rate; **the per-miss credit floor is the real cost driver to confirm.** |
| **Outreach / Teams** | Outreach **10,000/hr per user**; Teams **4 msg/sec per channel** | a few fires/day | **5+ orders of magnitude under.** Non-bottleneck. |
...
|---|---|---|
| **n8n** (orchestration glue) | **~€667/mo — Business tier** (annual billing) | Self-hosted is free, but a life-sciences buyer needs **SSO/SAML, RBAC, log-streaming**, which sit on the Business tier; Enterprise (quote-only) if they need more. *Proposed addition — fair-code license, not in your named stack.* |
...
| **Round-robin** | **$0 (Flow build)** or **~$45–59/user/mo** (app) | Native criteria-rules are free; true round-robin is a **Flow we build** or an app (Distribution Engine ~$45/user/mo min 5, or LeanData ~$59, contact-sales) at volume. |
