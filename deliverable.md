...
- **Country normalization is largely pre-solved** here (clean ISO + name columns agree) — the real identity risk is the 31 contact-less rows, which is where we spent the effort. The brief's own example (`US` → `United States`) is already two clean, consistent columns in this file: we treat the ISO `Country/Region` column as authoritative and drop `Country/Region Name` as the redundant display copy. The alias-normalizer the brief asks for **is in the code** as an explicit `normalize_country()` map — verifiably a **no-op** on this data (there's no "USA / U.S." mess to reconcile, and old-vs-new output is byte-identical), but present so the moment a second source sends free-text country names they collapse to the same ISO code instead of fragmenting geo routing.
...
- **LinkedIn Sales Navigator — a named tool, scoped to what it's best at.** Sales Nav isn't a *bulk* programmatic enrichment API here (ZoomInfo/Lusha own bulk firmographics and dials; LinkedIn's terms bar automated scraping). It plays two roles: (1) the rep's **manual identity/persona check** on buying-committee fit and **ABM list-building** that feeds the target-account watchlist — human judgment you don't automate; and (2) where native automation *is* sanctioned (no scraping), **CRM Sync** (activity writeback) and **Data Validation** (job-change / out-of-date contact flags synced into Salesforce, premium tiers) push persona-hygiene data into the CRM — a fast-follow, not day-one. That same manual surface is also Option 1's last-resort identity step on the 31 domain-only / 28 consumer-email rows. A named node with a deliberate manual/automated line — not a silent omission.
...
**Data-transfer / payload overhead — the dimension behind the call-count.** Both batches move kilobytes-to-low-MB, not a per-event stream: the **Option 1** webinar batch is ~100 rows × ~1–2 KB JSON (**~100–200 KB/run**), and the **Option 2** score batch — ~2,000 watched accounts, synced **changed-rows-only** because the score is **memoryless** (re-derived per batch, no carried state — §2.2) — tops out near **~4 MB even at a full recompute**. **The binding constraint is API call *count*, not bandwidth** — which is why the table above is framed in calls.
...
**The Data Cloud spec — what we stand up the day that threshold trips.** "No CDP today" is only credible if the day-it-flips plan already exists, so here it is: **Salesforce Data Cloud** (rebranded *Data 360* in late 2025; rung 4 of the native ladder, *their own* CDP — not a third-party Segment-class buy), scoped to identity only.

- **Ingest → model.** Each source — Salesforce Accounts/Contacts, ZoomInfo, Lusha, Gong-derived accounts, form fills — lands in its own **Data Lake Object (DLO)**, mapped to the standard **Data Model Objects (DMOs)**: `Individual` + `Account`.
- **Identity resolution.** **Match rules** link records into a **Unified Individual / Unified Account** profile; **reconciliation rules** (Data Cloud's native term for field-level survivorship) decide which value wins. The confidence tiers carry the same *detector-not-blind-resolver* posture as Option 1's `company_conflict`:

| Match signal | Confidence | Action |
|---|---|---|
| Exact email/website **domain** | High | Auto-unify |
| Fuzzy legal **name**, same country, no domain conflict | Medium | Unify, flag for steward audit |
| Conflicting domain **or** name+country mismatch | Low | Keep separate, queue for review |

*Match-key order — Company:* domain → legal name → `name + country`; *Person:* email → `name + company`. A wrong merge stays harder to make than a missed one.

- **Reconciliation priority (which field wins).** Ownership / lifecycle / pipeline → **Salesforce always wins** (system of record for revenue); firmographics (title, phone, employee count) → **most-recently-verified source wins**, priority **ZoomInfo > Lusha > form-fill**; **never** overwrite a human-edited Salesforce field with an automated source.
- **Writeback — an explicit build, not automatic.** A **Data Cloud-triggered Flow** (or Data Action) copies the unified-profile ID onto a read-only external-ID field (`DataCloud_Unified_Id__c`) on Account and Contact, plus a "canonical account" lookup on duplicates pointing at the golden record — the unified ID does *not* auto-populate out of the box. Routing and attribution then key off the **unified ID instead of the raw domain**.
- **Sync direction.** Source → Data Cloud (ingest; scheduled/streaming); Data Cloud → Salesforce (unified ID + reconciled firmographics, on resolution). Data Cloud is the **identity layer only** — it never owns ownership, pipeline, or activity, and the §2.3 sync map is unchanged beneath it (Salesforce stays the hub; Outreach/TitanX/Gong still sync up, now keyed off the unified ID). n8n still owns the activation fan-out; intent still accelerates-never-sources. Data Cloud changes exactly one thing: *which records are the same company.*
...
**Why n8n and not native Salesforce Flow (the question this table invites).** Flow is the right tool *inside* Salesforce's boundary — the round-robin row above is exactly that use. But the §2.2 activation lives *outside* that boundary: surge detection on intent feeds plus a fan-out to Teams, Outreach, ZoomInfo/Lusha, and TitanX. Make Flow the hub and every one of those becomes an Apex callout carrying its own auth, retry, and governor-limit exposure, with activation routed back through a Flow hop that adds latency and moves the enforcement point *into* the CRM. n8n owns that fan-out where the connectors already live; Salesforce stays the system of record. The brief explicitly allows Make/Zapier (Option 1) — **n8n is that sanctioned middleware category**; we pick it over hosted Make/Zapier specifically for **self-hosting / data residency**, a real life-sciences requirement (the €667/mo Business tier — annual — is just where SSO/SAML, the compliance floor, lands). **Fallback if budget rules out n8n:** Flow + Platform Events on the Salesforce side and scheduled-batch enrichment — native, but it gives up the minutes-level surge SLA and absorbs the heavier Apex-callout surface as the cost.
